...
type UploadState =
  | { kind: "idle" }
  | { kind: "uploading" }
  | { kind: "success"; name: string; note?: string }
      ...
      <div className="min-h-dvh grid place-items-center bg-[var(--background)] px-4">
        <div className="fridge-frame fridge-surface w-full max-w-md p-7">
          ...
          <p className="mt-2 text-sm text-zinc-600">
            Thanks for leaving a little memory behind, {state.name}.
          </p>
          {state.note ? (
        ...
        <div className="mx-auto w-full max-w-md">
          <div className="fridge-frame fridge-surface p-6">
            ...
            <form onSubmit={onSubmit} className="mt-6 flex flex-col gap-4">
              <div className="rounded-2xl border border-zinc-200 bg-white/65 p-4">
                <label className="text-sm font-medium text-zinc-800">
                  ...
                  <input
                    ...
                    className="mt-2 block w-full rounded-xl border border-zinc-200 bg-white/70 px-3 py-2.5 text-base outline-none focus:ring-2 focus:ring-zinc-900/15"
                  />
                </label>
              </div>
              <div className="rounded-2xl border border-zinc-200 bg-white/65 p-4">
              ...
              <div className="rounded-2xl border border-zinc-200 bg-white/65 p-4">
                <label className="text-sm font-medium text-zinc-800">
                  ...
                  <textarea
                    ...
                    rows={3}
                    className="mt-2 block w-full rounded-xl border border-zinc-200 bg-white/70 px-3 py-2.5 text-base outline-none focus:ring-2 focus:ring-zinc-900/15 resize-y"
                  />
                </label>
              </div>
              ...
              <button
                type="submit"
                disabled={!name.trim() || !file || state.kind === "uploading"}
